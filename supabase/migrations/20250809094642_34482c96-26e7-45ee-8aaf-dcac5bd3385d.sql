-- Create tables for referral program and secure function to record referrals
-- 1) referral_codes: one per user (or multiple), unique code, usage stats
-- 2) referrals: log referral relationships
...
-- RLS policies: owners manage their own codes
CREATE POLICY IF NOT EXISTS "referral_codes_select_own"
ON public.referral_codes
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "referral_codes_insert_own"
ON public.referral_codes
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "referral_codes_update_own"
ON public.referral_codes
FOR UPDATE
USING (auth.uid() = user_id);
...
-- RLS: allow both parties to view their referrals; no direct insert/update/delete policies (handled via function)
